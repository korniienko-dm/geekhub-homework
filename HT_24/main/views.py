...
from .tasks import parse_and_save_product
...
def add_products(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                for product_id in product_ids:
                    parse_and_save_product.apply_async(args=[product_id])
            except Exception as error:
                logger.error(f"Error processing product: {error}", exc_info=True)
                messages.error(request, "An error occurred while processing the products.")
        ...
        form = ProductForm()
    return render(request, 'main/add_products.html', {'form': form})
